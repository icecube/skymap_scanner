...
# find the GCD file
GCD_file = glob.glob(os.path.join(indir, "Level2pass2_*_GCD.i3.zst"))
if len(GCD_file) == 0:
    GCD_file = glob.glob(os.path.join(indir, "Level2pass2_*_GCD.i3.gz"))
if len(GCD_file) == 0:
...
run_files = sorted(glob.glob(os.path.join(indir, "Level2pass2_????.????_data_Run????????_Subrun00000000_????????.i3.zst")))
