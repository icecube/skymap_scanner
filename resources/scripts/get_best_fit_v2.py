...
def get_best_fit_v2(eventID, cache_dir, log_func=None):
    ...
    text = "Best fit stored in " + os.path.join(cache_dir, eventID, "nside{0:06d}".format(best_nside), "pix{0:012d}.i3".format(best_pix)) 
    text += " with best-fit energy = {0} TeV".format(best_energy/I3Units.TeV) 
    log_func(text)
