...
from ..utils import get_event_mjd, parse_event_id
...
def create_plot(event_id_string, state_dict):
    ...
    if grid_map is None:
        ...
        grid_pix = healpy.ang2pix(8, DEC + numpy.pi/2., RA)
        this_map = numpy.ones(healpy.nside2npix(8))*numpy.inf
        grid_map = this_map[grid_pix]
        del this_map
        del grid_pix
    ...
    ax.clabel(CS, inline=False, fontsize=4, fmt=dict(list(zip(contour_levels, contour_labels))))
