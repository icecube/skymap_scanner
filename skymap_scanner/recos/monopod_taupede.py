...
class MonoTau(RecoInterface):
    ...
    def prepare_frames(self, tray, name, logger, **kwargs) -> None:
        ...
        tray.Add(
            extract_seed, "ExtractSeed", If=lambda frame: frame.Has("HESE_VHESelfVeto")
        )
        ...
        tray.Add(
            mask_deepcore,
            origpulses=self.pulsesName_input,
            maskedpulses=self.pulsesName,
        )
    ...
    def exclusions(self, tray, name):
        ...
        tray.AddModule(
            createEmptyDOMLists, "createEmptyDOMLists", ListNames=["BrightDOMs"]
        )
        # exclude bright DOMs
    ...
    def traysegment(self, tray, name, logger, seed):
        ...
        def check_cal(frame):
            ...
            for omkey in list(cal.dom_cal.keys())[::100]:
                ...
                logger.debug(
                    f"......: {x.combined_spe_charge_distribution.compensation_factor}"
                )
        ...
        icetray.logging.log_info(pformat(millipede_params), __name__)
        minis = [_ for _ in ["MIGRAD", "iMIGRAD", "SIMPLEX", "iSIMPLEX", "LBFGSB"]]
        sfx = "PPB0"
        for mini in minis:
            tray.Add(
                wrapperfn,
        ...
        del millipede_params[
            "PhotonsPerBin"
        ]  # also input to MillipedeWrapper next, gives error if entered twice

        # HESE millipede
        tray.Add(
            MillipedeWrapper,
            f"HESEMillipedeFit_{sfx}",
        ...
        tray.Add(
            "Rename",
            Keys=[
                "SRTSplitInIcePulses_IC_Singles",
                f"SRTSplitInIcePulses_IC_Singles_{sfx}",
                "PreferredFit_key",
                f"PreferredFit_key_{sfx}",
                "PreferredFit",
                f"PreferredFit_{sfx}",
            ],
        )
