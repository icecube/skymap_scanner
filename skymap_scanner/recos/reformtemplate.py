...
class MonopodTaupede(RecoInterface):
    ...
    def TaupedeWrapper(self, tray, name, Seed, Iterations=1, Chain=1, **params):
        ...
        if Chain:
            ...
            tray.Add(self.MonopodWrapper, name, Seed=Seed, Unfold=False, Chain=Chain, **params)
    ...
    def to_recopixelvariation(cls, frame: I3Frame, geometry: I3Frame) -> RecoPixelVariation:
        ...
        return RecoPixelVariation(
            ...
            position=frame['TaupedeFit_FinalFitParticles'].pos,
            time=frame['TaupedeFit_FinalFitParticles'].time,
            energy=frame['TaupedeFit_FinalFitParticles'].energy,
        )

    @staticmethod
    def get_reco_losses_inside(p_frame: I3Frame, g_frame: I3Frame) -> Tuple[float, float]:

        if 'TaupedeFit_FinalFitParticles' not in p_frame:
            return numpy.nan, numpy.nan
        recoParticle = p_frame['TaupedeFit_FinalFitParticles']

        if 'TaupedeFit_FinalFitParams' not in p_frame:
            return numpy.nan, numpy.nan
        ...
        def getRecoLosses(vecParticles):
            ...
            return losses
        recoLosses = getRecoLosses(p_frame['TaupedeFit_FinalFitParams'])
